...
const ManageParty = () => {
  ...
  const [initialForm, setInitialForm] = useState<PartyForm>(emptyForm);
  ...
  useEffect(() => {
    ...
    const loadCustomer = async () => {
      ...
      setIsLoading(true);
      try {
        const { response, data } = await apiJson<{ party?: any }>(
        ...
        const nextForm: PartyForm = {
          firstName: data.party.firstName ?? "",
          lastName: data.party.lastName ?? "",
          email: data.party.email ?? "",
          phone: data.party.phone ?? "",
          streetAddress: data.party.streetAddress ?? "",
          area: data.party.area ?? "",
          city: data.party.city ?? "",
          state: data.party.state ?? "",
          pinCode: data.party.pinCode ?? "",
          bankDetails: {
            accountNumber: data.party.bankDetails?.accountNumber ?? "",
            ifscCode: data.party.bankDetails?.ifscCode ?? "",
            bankName: data.party.bankDetails?.bankName ?? "",
            isBankVerified: Boolean(data.party.bankDetails?.isBankVerified ?? false),
            beneficiaryName: data.party.bankDetails?.beneficiaryName ?? "",
          },
        };
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        toast.error(data?.message || "Failed to update party");
        return;
      }
