...
router.get("/fetch-parties", async (req, res) => {
    try {
        const parties = await prisma_1.prisma.parties.findMany({
            select: {
                id: true,
                firstName: true,
                lastName: true,
...
router.post("/add-party", async (req, res) => {
    try {
        ...
        const created = await prisma_1.prisma.parties.create({
            data: {
                firstName,
                lastName,
                email,
...
router.get("/:partyId", async (req, res) => {
    try {
        ...
        const party = await prisma_1.prisma.parties.findUnique({
            where: { id: partyId },
            select: {
                id: true,
                firstName: true,
        ...
        const address = party.address ?? null;
...
router.put("/:partyId", async (req, res) => {
    try {
        ...
        const hasAddress = Boolean(streetAddress || area || city || state || pinCode);
        const bankDetailsBody = body.bankDetails ?? null;
        const updated = await prisma_1.prisma.parties.update({
            where: { id: partyId },
            data: {
                firstName,
                lastName,
