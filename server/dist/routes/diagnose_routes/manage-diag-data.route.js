...
async function handleGetDiagnostics(req, res) {
    try {
        ...
        const mtype = req.query.mtype ||
            req.query.type ||
            req.query.diagnoseType ||
            req.body.mtype ||
            req.body.type ||
            req.body.diagnoseType;
...
async function handleUpdateDiagnostics(req, res) {
    try {
        ...
        const updatedData = await diagnostics_service_1.DiagnosticsService.updateDiagnosticsDataByTestId(testId.trim(), updates, mtype);
        return res.status(constants_1.HTTP_STATUS.OK).json({
            success: true,
