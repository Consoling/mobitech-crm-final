...
router.post(`/upload-selfie-temp`, async (req: Request, res: Response) => {
  try {
    ...
    const uploadDir = path.join(process.cwd(), "temp_diag_selfies");
    if (!fs.existsSync(uploadDir)) {
      fs.mkdirSync(uploadDir, { recursive: true });
    }

    // Generate filename
    const filename = `${employeeId}-${uuidv4()}.${imageType}`;
