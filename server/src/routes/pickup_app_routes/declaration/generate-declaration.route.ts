...
router.post("/generate-declaration", async (req, res) => {
  try {
    ...
    const exisitingDeclaration = await prisma.declaration.findUnique({
      where: {
        orderId: orderId,
      },
    });
    ...
    if (!exisitingDeclaration) {
      ...
      if (!declaration) {
        ...
        let whatsappRes = fetch(
          `https://www.fast2sms.com/dev/whatsapp?authorization=${process.env.FAST2SMS_API_KEY
          }&numbers=+91${cphoneNumber}
          &message_id=7379&variables_values=${encodeURIComponent(
            variables_values
          )}`,
          {
            method: "GET",
        ...
        if (whatsappSuccess) {
          ...
          if (whatsappSuccess) sentMediums.push("WhatsApp");
