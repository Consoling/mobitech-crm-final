...
export class DiagnosticsService {
   ...
   */
  static normalizeToBoolean(value: any): boolean {
    if (value === null || value === undefined) return false;
    if (typeof value === "boolean") return value;
    if (typeof value === "number") return value !== 0;
    if (typeof value === "object") {
      if (value.status !== undefined) return this.normalizeToBoolean(value.status);
    }
   ...
   */
  static parseVariantRamRom(variant?: string | null): { ram: string; rom: string } {
  ...
  static formatMobitechDiagnostics(record: any) {
    ...
    const backCameraVal =
      cameraTest.backCamera !== undefined
    ...
    const audioJackVal =
      record.audioJackTest?.status !== undefined
    ...
    return {
      ...
      multiTouch: this.normalizeToBoolean(
        screenTest.multiTouchPassed !== undefined
   ...
   */
  static formatSelfDiagnostics(record: any) {
   ...
   */
  static async getFormattedDiagnosticsByTestId(testId: string, mtype?: string) {
    const normalizedType = mtype?.toLowerCase().trim();

    if (normalizedType === "sfdg" || normalizedType === "self") {
      const selfDiag = await prisma.manualDiagnosticsResult.findUnique({
        where: { diagnoseId: testId },
      });
      if (!selfDiag) return null;
      return this.formatSelfDiagnostics(selfDiag);
    }

    if (normalizedType === "mbdg" || normalizedType === "mobitech") {
      const mobitechDiag = await prisma.diagnosticsData.findUnique({
        where: { testId },
      });
      if (!mobitechDiag) return null;
   ...
   */
  static async updateSelfDiagnosticsDataByTestId(testId: string, updates: any) {
    const existing = await prisma.manualDiagnosticsResult.findUnique({
      where: { diagnoseId: testId },
    });

    if (!existing) {
      throw new ValidationError(`Self diagnostics data with diagnoseId '${testId}' not found`);
    }

    const toStatusString = (val: any) => (this.normalizeToBoolean(val) ? "Working" : "Not working");

    const updatedData: any = {};
    if (updates.imei1 !== undefined) updatedData.imei1 = updates.imei1;
    if (updates.imei2 !== undefined) updatedData.imei2 = updates.imei2;
    ...
    if (updates.bluetooth !== undefined) updatedData.bluetooth = toStatusString(updates.bluetooth);
    if (updates.gps !== undefined) updatedData.gps = toStatusString(updates.gps);
    if (updates.wifi !== undefined) updatedData.wifi = toStatusString(updates.wifi);
    if (updates.proximity !== undefined) updatedData.proximity = toStatusString(updates.proximity);
    ...
    if (updates.speaker !== undefined) updatedData.speaker = toStatusString(updates.speaker);
    if (updates.microphone !== undefined) updatedData.microphone = toStatusString(updates.microphone);
    if (updates.frontCamera !== undefined) updatedData.frontCamera = toStatusString(updates.frontCamera);
    if (updates.backCamera !== undefined) updatedData.backCamera = toStatusString(updates.backCamera);
    if (updates.chargingPort !== undefined) updatedData.chargingPort = toStatusString(updates.chargingPort);
    if (updates.fingerprint !== undefined) updatedData.fingerprint = toStatusString(updates.fingerprint);
    if (updates.audioJack !== undefined) updatedData.audioJack = toStatusString(updates.audioJack);
    ...
    const result = await prisma.manualDiagnosticsResult.update({
      where: { diagnoseId: testId },
      data: updatedData,
    });
   ...
   */
  static async updateDiagnosticsDataByTestId(testId: string, updates: any, mtype?: string) {
    ...
    const existingMobitech = await prisma.diagnosticsData.findUnique({
      where: { testId },
      select: { id: true },
    });
    ...
    const existingSelf = await prisma.manualDiagnosticsResult.findUnique({
      where: { diagnoseId: testId },
      select: { id: true },
    });
    ...
    throw new ValidationError(`Diagnostic record with testId/diagnoseId '${testId}' not found`);
  }

  /**
   * Internal implementation for updating Mobitech DiagnosticsData
   */
  static async updateMobitechDiagnosticsDataByTestId(testId: string, updates: any) {
    const existing = await prisma.diagnosticsData.findUnique({
      where: { testId },
    });

    if (!existing) {
      throw new ValidationError(`Mobitech diagnostics data with testId '${testId}' not found`);
    }

    const existingDeviceInfo: any = existing.deviceInfo || {};
    const existingCameraTest: any = existing.cameraTest || {};
    const existingScreenTest: any = existing.screenTest || {};
    const existingWifiTest: any = existing.wifiTest || {};
    const existingBluetoothTest: any = existing.bluetoothTest || {};
    const existingGpsTest: any = existing.gpsTest || {};
    const existingProximityTest: any = existing.proximityTest || {};
    const existingSpeakerTest: any = existing.speakerTest || {};
    const existingEarpieceTest: any = existing.earpieceTest || {};
    const existingMicrophoneTest: any = existing.microphoneTest || {};
    const existingFingerprintTest: any = existing.fingerprintTest || {};
    const existingUsbTest: any = existing.usbPortTest || {};
    const existingAudioJackTest: any = existing.audioJackTest || {};
    ...
    const result = await prisma.diagnosticsData.update({
      where: { testId },
      data: updatedData,
    });
